...
class App {
  ...
  _getPosition() {
    const defaultCoords = {
      latitude: 48.8566, // Default coordinates
      longitude: 2.3522,
    };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        // If access is granted, call _loadMap with the user's current coordinates
        this._loadMap.bind(this),
        // If access is denied, call _loadMap with default coordinates
        () =>
          this._loadMap({ coords: defaultCoords, zoomLevel: defaultZoomLevel })
      );
    } else {
  ...
  _loadMap(position) {
    ...
    const zoomLevel = position.zoomLevel || this.#mapZoomLevel; // Use default zoom level if none is provided
  ...
  _renderWorkoutMarker(workout) {
    ...
    // Store the marker in the dictionary with workout ID as the key
    this.#markers[workout.id] = marker;
  }

  _removeWorkoutMarker(workoutId) {
    // Remove the marker from the map
    if (this.#markers[workoutId]) {
      this.#map.removeLayer(this.#markers[workoutId]);
      delete this.#markers[workoutId]; // Also remove from the dictionary
    }
  }
  ...
  _renderWorkout(workout) {
    ...
    <li class="workout workout--${workout.type}" data-id="${workout.id}">
          <button type="button" class="form__close-btn">&times;</button>
    ...
    document
      .querySelector(`[data-id="${workout.id}"] .form__close-btn`)
